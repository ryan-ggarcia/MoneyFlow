class Despesas_Model
  attr_accessor :des_id, :des_nome, :des_valorUnitario, :des_data, :des_parcelaNu, :usu_id, :cat_id, :con_id, :cat_id, :fat_id,
                :com_id
  ...
  def self.insert
    sql = `INSERT INTO despesas (des_nome,des_valorUnitario,des_data,des_parcelaNu,usu_id,cat_id,con_id,fat_id,com_id)
      VALUES (?,?,?,?,?,?,?,?,?)`
    Database.executa_comando(sql, @des_nome, @des_valorUnitario, @des_data, @des_parcelaNu, @usu_id, @cat_id, @con_id, @fat_id,
                             @com_id)
  end
